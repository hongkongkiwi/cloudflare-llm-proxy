...
describe('Groq API Integration Tests', () => {
  ...
  describe('Groq Chat Completions Endpoint', () => {
    ...
    it('should handle Groq models endpoint correctly', async () => {
      ...
      const mockCall = mockFetch.mock.calls[0];
      const mockRequest = mockCall[0];
      expect(mockRequest.url).toBe('https://api.groq.com/v1/models');
  ...
  describe('Groq Error Handling', () => {
    it('should handle Groq rate limiting', async () => {
      mockFetch.mockResolvedValueOnce(new Response(JSON.stringify({
        error: {
  ...
  describe('Groq Request Validation', () => {
